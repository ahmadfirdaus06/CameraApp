package com.ahmad.cameraapp.config;
...
public class Cache {
    ...
    public void deleteImageFolder(File fileOrDirectory) {

        if (fileOrDirectory.isDirectory()) {
            for (File child : fileOrDirectory.listFiles()) {
                deleteImageFolder(child);
            }
        }
    ...
    public boolean removeAllReportCache(){
        ...
        if (removeStudentInfoCache() && removeExamInfoCache() && removeEvidenceDetailsCache() && removeSubReportDetailsCache() && removeApprovalDetailsCache() && removeChiefInvDetailsCache()){
            String path = "/storage/emulated/0/Android/data/com.example.cameraapp/files/";
            File dir = new File(path);
            deleteImageFolder(dir);
            return true;
        }
        else{
            return false;
